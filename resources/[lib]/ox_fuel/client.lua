...
-- 			-- 	print("Fuel non défini après plusieurs tentatives. 2")
-- 			-- 	return
-- 			-- end
...
lib.onCache('seat', function(seat)
    ...
    if seat == -1 then
        SetTimeout(0, function()
            ...
            -- Initialisation manuelle de 'fuel' si non défini
            if not state.fuel then
                local defaultFuelLevel = GetVehicleFuelLevel(vehicle)
                TriggerServerEvent('ox_fuel:createStatebag', NetworkGetNetworkIdFromEntity(vehicle), defaultFuelLevel)
                state.fuel = defaultFuelLevel
            end
