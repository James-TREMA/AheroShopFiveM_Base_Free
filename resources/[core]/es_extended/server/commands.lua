...
ESX.RegisterCommand('setjob2', 'admin', function(xPlayer, args, showError)
	...
	arguments = {
		{ name = 'playerId', help = TranslateCap('commandgeneric_playerid'), type = 'player' },
		{ name = 'job',      help = TranslateCap('command_setjob2_job'),      type = 'string' },
...
ESX.RegisterCommand('car', 'admin', function(xPlayer, args, showError)
	...
	ESX.OneSync.SpawnVehicle(args.car, playerCoords, playerHeading, upgrades, function(networkId)
		if networkId then
			...
			-- Vérifie si un PNJ est dans le siège du conducteur et le supprime si nécessaire
			...
			if GetVehiclePedIsIn(playerPed, false) ~= vehicle then
				showError("[ERREUR] Le joueur n'a pas pu être placé dans le véhicule")
			end
		end
	end)
end, false, {
...
ESX.RegisterCommand('group', {"admin" }, function(xPlayer, _, _)
	print(('%s, you are currently: ^5%s^0'):format(xPlayer.getName(), xPlayer.getGroup()))
end, true)

ESX.RegisterCommand('job', {"admin" }, function(xPlayer, _, _)
	print(('%s, your job is: ^5%s^0 - ^5%s^0'):format(xPlayer.getName(), xPlayer.getJob().name,
		xPlayer.getJob().grade_label))
end, false)

ESX.RegisterCommand('job2', {"admin" }, function(xPlayer, _, _)
	print(('%s, your job2 is: ^5%s^0 - ^5%s^0'):format(xPlayer.getName(), xPlayer.getJob2().name,
		xPlayer.getJob2().grade_label))
end, false)
...
ESX.RegisterCommand('info', {"user", "admin"}, function(xPlayer, args, showError)
    ...
    print("^2ID : ^5" .. xPlayer.source .. " ^0| ^2Name:^5" .. xPlayer.getName() .. " ^0 | ^2Group:^5" .. xPlayer.getGroup() .. " ^0 | ^2Job:^5" .. job .. " ^0 | ^2Job Grade:^5" .. jobgrade .. " ^0 | ^2Job2:^5" .. job2 .. " ^0 | ^2Job2 Grade:^5" .. job2grade .. "^0")
end, true)
