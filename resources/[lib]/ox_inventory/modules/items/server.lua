...
-- Métadonnées possibles lors de la création de déchets
local trash = {
	{description = 'Un vieux journal roulé.', weight = 200, image = 'trash_newspaper'},
	{description = 'Un carton de Burger Shot jeté.', weight = 50, image = 'trash_burgershot'},
	{description = 'Une canette de soda vide.', weight = 20, image = 'trash_can'},
	{description = 'Un morceau de pain moisi.', weight = 70, image = 'trash_bread'},
	{description = 'Un paquet de cigarettes vide.', weight = 10, image = 'trash_fags'},
	{description = 'Une paire de culottes légèrement utilisée.', weight = 20, image = 'panties'},
	{description = 'Un gobelet de café vide.', weight = 20, image = 'trash_coffee'},
	{description = 'Un morceau de papier froissé.', weight = 5, image = 'trash_paper'},
	{description = 'Un sachet de chips vide.', weight = 5, image = 'trash_chips'},
}
...
CreateThread(function()
	...
	[%q] = {
		...
		description = %q
				...
				shared.info(count, 'éléments ont été copiés de la base de données.')
				shared.info('Vous devriez redémarrer la ressource pour charger les nouveaux éléments.')
			end

			shared.info('La base de données contient', #items, 'éléments.')
		end
	...
	[%q] = {
		...
		client = {
			...
			image = %q,
				...
				SaveResourceFile(shared.resource, 'data/items.lua', table.concat(file), -1)
				shared.info(count, 'éléments ont été copiés depuis QBCore.Shared.Items.')
				shared.info('Vous devriez redémarrer la ressource pour charger les nouveaux éléments.')
			end
		end
	...
	shared.info(('L’inventaire a chargé %d objets'):format(count))
...
function Items.Metadata(inv, item, metadata, count)
	...
	if metadata.imageurl and Utils.IsValidImageUrl then
		if Utils.IsValidImageUrl(metadata.imageurl) then
			Utils.DiscordEmbed('URL d\'image valide', ('Création de l\'élément "%s" (%s) avec une URL valide dans "%s".\n%s\nid : %s\npropriétaire : %s'):format(metadata.label or item.label, item.name, inv.label, metadata.imageurl, inv.id, inv.owner, metadata.imageurl), metadata.imageurl, 65280)
		else
			Utils.DiscordEmbed('URL d\'image invalide', ('Création de l\'élément "%s" (%s) avec une URL invalide dans "%s".\n%s\nid : %s\npropriétaire : %s'):format(metadata.label or item.label, item.name, inv.label, metadata.imageurl, inv.id, inv.owner, metadata.imageurl), metadata.imageurl, 16711680)
