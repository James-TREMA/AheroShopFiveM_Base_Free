...
function createESXPlayer(identifier, playerId, data)
	...
	print(("Un nouveau utilisateur a été créé sous le nom de ^5%s^7. Le rank attribué est ^2utilisateur^0."):format(GetPlayerName(playerId)))

	local parameters = Config.Multichar and { json.encode(accounts), identifier, "user", data.firstname, data.lastname, data.dateofbirth, data.sex, data.height } or { json.encode(accounts), identifier, defaultGroup }
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.job2, userData.loadout, userData.playerName, userData.coords, userData.metadata)
	...
	print(("[^3INFO^0] ^7Le joueur ^5\"%s\"^0 s'est connecté au serveur. ID : ^2%s^7"):format(xPlayer.getName(), playerId))
end
