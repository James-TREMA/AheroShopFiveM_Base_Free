...
function ESX.RegisterCommand(name, group, cb, allowConsole, suggestion)
	...
	RegisterCommand(name, function(playerId, args)
		...
		if not command.allowConsole and playerId == 0 then
			...
			-- Mise en place de la vérificatuiion de la permission, si c'est un "user" qui tente d'utiliser une commande admin, on lui affiche un message d'erreur
			if xPlayer and xPlayer.getGroup() == 'user' then
				xPlayer.showNotification("[ERROR] Vous n'avez pas la permission d'utiliser cette commande.")
				return
			end
	...
	MySQL.prepare(
		...
		function(affectedRows)
			if affectedRows == 1 then
				print(('[^2INFO^7] Joueur sauvegardé ^5"%s^7"'):format(xPlayer.name))
...
function Core.SavePlayers(cb)
	...
	MySQL.prepare(
		...
		function(results)
			...
			print(('[^2INFO^7] Sauvegardé ^5%s^7 %s en ^5%s^7 ms'):format(#parameters, #parameters > 1 and 'joueurs' or 'joueur', ESX.Math.Round((os.time() - startTime) / 1000000, 2)))
		end
	)
end
...
function ESX.RefreshJobs()
	...
	if not Jobs then
		...
		ESX.Jobs['unemployed'] = { label = 'Chômage', grades = { ['0'] = { grade = 0, label = 'Chômage', salary = 200, skin_male = {}, skin_female = {} } } }
	else
		ESX.Jobs = Jobs
	end
end
...
function ESX.RefreshJobs2()
	...
	local jobs2Grades = MySQL.query.await('SELECT * FROM job2_grades')
  
	for _, v in ipairs(jobs2Grades) do
	...
	if not Jobs2 then
		-- Fallback data, if no job2 exist
		ESX.Jobs2['unemployed2'] = {label = 'Organisation', grades = {['0'] = {grade = 0, label = 'Aucun', salary = 0, skin_male = {}, skin_female = {}}}}
	else
		ESX.Jobs2 = Jobs2
	end
end
...
-- job2
ESX.DoesJob2Exist = function(job, grade)
	grade = tostring(grade)

	if job and grade then
		if ESX.Jobs2[job] and ESX.Jobs2[job].grades[grade] then
			return true
		end
	end
